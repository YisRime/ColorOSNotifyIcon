...
package de.yisrime.cosicon.hook.entity
...
import de.yisrime.cosicon.hook.entity.SystemUIHooker.NOTIFY_ICON_PADDING_RATIO_MD3
import de.yisrime.cosicon.hook.entity.SystemUIHooker.OplusNotificationHeaderViewWrapperExImpClass
import de.yisrime.cosicon.param.factory.isAppNotifyEnabledOf
import de.yisrime.cosicon.param.factory.isAppNotifyOverlayOf
import de.yisrime.cosicon.utils.factory.appIconOf
import de.yisrime.cosicon.utils.factory.appNameOf
import de.yisrime.cosicon.utils.factory.colorAlphaOf
import de.yisrime.cosicon.utils.factory.delayedRun
import de.yisrime.cosicon.utils.factory.dpFloat
import de.yisrime.cosicon.utils.factory.drawableOf
import de.yisrime.cosicon.utils.factory.isSystemInDarkMode
import de.yisrime.cosicon.utils.factory.isUpperOfAndroidS
import de.yisrime.cosicon.utils.factory.runInSafe
import de.yisrime.cosicon.utils.factory.safeOf
import de.yisrime.cosicon.utils.factory.safeOfFalse
import de.yisrime.cosicon.utils.factory.safeOfNull
import de.yisrime.cosicon.utils.factory.stampToDate
import de.yisrime.cosicon.utils.factory.systemAccentColor
import de.yisrime.cosicon.utils.tool.ActivationPromptTool
import de.yisrime.cosicon.utils.tool.BitmapCompatTool
import de.yisrime.cosicon.utils.tool.IconAdaptationTool
...
import de.yisrime.cosicon.hook.any
import de.yisrime.cosicon.hook.cast
import de.yisrime.cosicon.hook.hookAll
import de.yisrime.cosicon.hook.hook
import de.yisrime.cosicon.hook.int
import de.yisrime.cosicon.hook.string
import de.yisrime.cosicon.hook.toClass
import de.yisrime.cosicon.data.ConfigStore
import de.yisrime.cosicon.hook.injectModuleAppResources
import de.yisrime.cosicon.utils.tool.ModuleLog
import de.yisrime.cosicon.hook.callHostMethod
import de.yisrime.cosicon.hook.readHostField
 ...
 * 系统界面核心 Hook 类
 */
object SystemUIHooker : HookerBase() {
    ...
    private fun loggerDebug(tag: String, context: Context, nf: StatusBarNotification?, isCustom: Boolean, isGrayscale: Boolean, contextPackageName: String, iconColor: Int) {
        if (ConfigData.isEnableModuleLog) ModuleLog.debug(
            msg = "(Processing $tag) ↓\n" +
    ...
    private fun refreshStatusBarIcons() = runInSafe {
        ...
        val roundUtil = RoundRectDrawableUtil_CompanionClass.resolve().optional(silent = true).firstMethodOrNull {
            ...
            if (this == null) ModuleLog.error("Your system not support \"getRoundRectDrawable\"!")
            ...
            (notificationIconContainer?.children?.toList() ?: notificationIconInstances.takeIf { it.isNotEmpty() })?.forEach {
                runInSafe {
                    ...
                    val iconDrawable = nf.notification.smallIcon.loadDrawable(it.context)
                        ?: return@Thread ModuleLog.warn("refreshStatusBarIcons got null smallIcon")
                    /** 获取优化后的状态栏通知图标 */
                    compatStatusIcon(
                        context = it.context,
                        nf = nf,
    ...
    private fun headerIconOf(headerWrapperExImp: Any?) =
        safeOfNull { headerWrapperExImp.callHostMethod("getBase").readHostField("mIcon") as? ImageView }
    ...
    private fun styleHeaderIcon(headerWrapperExImp: Any?) = runInSafe {
        val base = headerWrapperExImp?.let { it.callHostMethod("getBase") } ?: return@runInSafe
        val iconView = base.readHostField("mIcon") as? ImageView ?: return@runInSafe
    ...
    private fun registerLifecycle() {
        ...
        /** 刷新图标缓存 */
        SystemUITool.Host.onRefreshSystemUI() { recachingPrefs(it) }
        /** 模块进程只传递刷新命令，SystemUI 使用自身缓存获取 ANIP 资源 */
        SystemUITool.Host.onRefreshIconData() { version, result ->
    ...
    private fun refreshIconData(
        ...
        iconRuleScope.launch {
            ...
            val fetchResult = runCatching {
                ...
                ModuleLog.error("Failed to refresh ANIP icon resources", it)
                result(false)
            ...
            if (isNotifyResult) runCatching { IconRuleManagerTool.notifyFetchResult(context, fetchResult) }
                .onFailure { ModuleLog.error("Failed to notify icon resource update", it) }
        }
    }
    ...
    private fun recachingPrefs(isRefreshCacheOnly: Boolean = false): Boolean {
        ...
        /** 获取可读写状态 */
        return ConfigStore.isPreferencesAvailable.also {
