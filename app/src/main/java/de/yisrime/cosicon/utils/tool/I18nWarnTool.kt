...
object I18nWarnTool {
    ...
    /** 推广已读存储键值 */
    private val LOCALE_WARN_READED = PrefKey("locale_warn_readed", false)
    ...
    fun checkingOrShowing(context: Context) {
        fun saveReaded() = ConfigStore.put(LOCALE_WARN_READED, true)
        if (Locale.getDefault().language.startsWith("zh").not() && ConfigStore.get(LOCALE_WARN_READED).not())
            context.showDialog {
